...
export const DetailsPages = () => {
  ...
  // step 3
  const PublicFlo = "http://localhost:8080/images/"
  const { user } = useContext(Context)

  const handleDelete = async () => {
    try {
      await axios.delete(`http://localhost:8080/posts/${post._id}`, { data: { username: user.username } })
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`http://localhost:8080/posts/${post._id}`, { username: user.username, title, desc })
